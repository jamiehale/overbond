...
module Overbond
  ...
  describe BondFileLoader do
    ...
    describe 'once created' do
      ...
      it 'calls gets for the header line and then each_line for remainder of file' do
      ...
      describe 'with a valid header' do
        ...
        before( :each ) { allow( stream ).to receive( :gets ).and_return( valid_header ) }
        ...
        it 'returns an empty array if the passed stream has no bonds' do
          allow( stream ).to receive( :each_line )
          expect( loader.load( stream ) ).to be_empty
        end

        it 'returns a single element array if the stream has a single well-formed line' do
        ...
        it 'returns multiple elements if the stream has multiple well-formed lines' do
      ...
      describe 'error conditions' do
        ...
        it 'raises an error if the stream has no header' do
          allow( stream ).to receive( :gets ).and_return( 'C1,corporate,1.3 years,3.30%' )
          expect{ loader.load( stream ) }.to raise_error( OverbondException )
        end
